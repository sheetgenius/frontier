...
description: >-
  ...
  rare decision-bearing signals, and (for a weekly window) publish the digest. Use
...
3. **Released is not merged.** Resolve and state the release-channel status of
   every change by **git ancestry, not date**: `tagged-release` (the commit is in a
   stable tag's history), `main-unreleased` (on the default branch, in no tag), or
   `preview-or-beta` (only in a `-alpha`/`-beta`/`-rc` tag). The gap between merged
   and released is where an operator gets a false sense of safety -- it is usually
   the story.
...
The established harness is one **coordinator** (final editorial authority) plus
**parallel Opus researchers at max effort**, one per source or small group. Use the
Workflow tool or parallel Agent calls; commit frequently, cross-linking commit
messages to `JOURNAL.md` entries. Each researcher:
...
### 6. Digest (weekly)
Author `content/digests/<start>_<end>-weekly.md` on the fixed five beats: a single
