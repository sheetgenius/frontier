...
operator_brief:
  ...
  upgrade_check:
    - "Gemini CLI: upgrade past v0.53.0 before pointing a2a-server at a repository you did not write. On earlier builds a hostile workspace could ship a .gemini/.env setting GEMINI_CLI_TRUST_WORKSPACE=true and self-validate before the trust check ran; the fix states the class as zero-click RCE. https://github.com/google-gemini/gemini-cli/pull/28470"
  ...
  watch:
    ...
    - "Whether Omnigent's spend cap grows a genuine stop. A downgrade gate is a real control and is enforced pre-call, but it is not the ceiling the parameter name implies, and Omnigent is the first project on the watchlist whose whole product is governance over other harnesses. https://github.com/omnigent-ai/omnigent/blob/v0.7.0/omnigent/policies/builtins/cost.py"
  ...
  uncertain:
    ...
    - "Whether Omnigent's voice dictation keeps audio on infrastructure the operator controls. The release says audio never leaves your server while describing the transcription engine as offloadable to a remote worker, and does not say which setup the first sentence covers. https://github.com/omnigent-ai/omnigent/releases/tag/v0.7.0"
    - "What Omnigent's policy layer does when it disagrees with the permission system of the harness it is driving. It is the question Omnigent joined the watchlist to answer, and it still has no public answer. https://github.com/omnigent-ai/omnigent"
...
Every project here has a newest thing it has
...
entirely when untrusted, and adds task-level isolation. It reached
...
The same stable shipped thread forks, and a Korean-language walkthrough of it
drew the line the release notes leave implicit:
[[q:easyworld-fork-is-not-a-worktree]] In our rendering: do not treat branching
the conversation as separating the code. The notes describe forking thread
history and nothing else, so two forks that both edit files are editing one
working tree unless each gets its own.
...
Hermes's maintainer, who says [[q:teknium-hundred-percent]], is shipping fast
...
## Omnigent found its own gap in six days

Omnigent joins the watchlist this week as its fifteenth project and its first
...
framework counted, Flue's maintainer said [[q:schott-meta-harness-boundary]] --
better evidence that the boundary is real and unsettled than any definition this
publication could write. Minutes later, in the same exchange, he described the
stacking happening anyway, as a pattern he had heard of rather than measured:
[[q:schott-flue-delegates-coding]].

Builders are assembling it by hand. One HumanLayer builder's list of every setup
he has used ends with [[q:dexhorthy-fables-managing-codex]], and closes by
saying he still reads the code. The opposite instinct was in the window too. The
Agent Flywheel author published a rule for his own swarms, drafted by his agent
after, by the rule's own account, agents spent hours on evidence machinery while
product functionality stayed missing. It forbids [[q:emanuel-harness-for-a-harness]]. It is not about Omnigent. It is the
cost any governance layer has to earn back.
...
When Omnigent joined the watchlist, the first question we wrote down was
whether its spend caps are enforced before a call is made or reconciled after
it, on the grounds that a cap which reconciles is a report rather than a
control. This window the code answered it, and the answer is more interesting
than either option.

It enforces before, at two points. In
[the module at the tag](https://github.com/omnigent-ai/omnigent/blob/v0.7.0/omnigent/policies/builtins/cost.py),
...
[[q:omnigent-stateful-policies-claim]]. That one is checkable and it checks out:
...
[[q:omnigent-audio-stays-on-your-server]] for the new voice dictation, while the
release notes describe the same transcription engine as offloadable to a remote
worker. Which of those an operator actually gets is the
part nobody has published.
...
[[q:hallie-fork-subtask]], with the old in-session behaviour moving to
...
[[q:cherny-actively-working]].

And he was doing something else, which is the sharpest thing anyone said in this
window about words:

<!--card:cherny-do-not-attribute-->

Operators kept using the floor, and filled the quiet with their own reads.
One, running a Gradle-to-APK-to-Pixel loop over ADB against Cursor's CLI agent,
reported that Claude Code ran the whole path and concluded that
[[q:chris-a-harness-over-model]]. Another asked whether the usage
allowance had been cut, reporting that [[q:santana-usage-halved]] We could not
...
would otherwise reach logs and referrers. One more number arrived that week, when a
researcher announced [[q:xuli-toolshield-since-v1-36]] ToolShield judges a tool
call's risk with a separate guardrail model rather than the model that proposed
the call. That `v1.36.0` belongs to OpenHands' agent SDK, which
[shipped it on July 13](https://github.com/OpenHands/software-agent-sdk/releases/tag/v1.36.0),
not to the repository whose Latest is `v1.8.0`. One project name, three version
lines.
...
the fact: [[q:openai-codex-security-cli]].
...
**Gemini CLI**'s news is the trust fix above; the rest of its window was
routine stable, preview and nightly traffic.

**eve** tagged `0.27.9` through `0.29.5`, none of it changing an operator's read.
...
This window it broke four times. One published a
