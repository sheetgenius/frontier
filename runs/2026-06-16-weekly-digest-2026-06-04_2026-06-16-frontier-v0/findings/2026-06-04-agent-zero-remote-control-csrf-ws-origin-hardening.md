---
schema_version: bitter.frontier_finding.v0
finding_id: 2026-06-04-agent-zero-remote-control-csrf-ws-origin-hardening
source: agent-zero
source_contract: sources/agent-zero.yml
window:
  start: 2026-06-04
  end: 2026-06-16
status: accepted
confidence: high
evidence:
  - url: https://github.com/agent0ai/agent-zero/commit/ca4efe6e6
    precision: release_note
---
# 2026-06-04-agent-zero-remote-control-csrf-ws-origin-hardening

Index stub for the 2026-06-04..2026-06-16 run. Full receipted detail (what
changed, operator implication, accessibility and security consequence, and
release-channel status) lives in harvest/agent-zero.md, with the curated signal in
signals/frontier-signals.yml.

## Receipt
- https://github.com/agent0ai/agent-zero/commit/ca4efe6e6
